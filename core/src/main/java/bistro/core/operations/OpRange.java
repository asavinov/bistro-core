...
public class OpRange implements Operation {
    ...
    void validate() {
        ...
        if(this.rangeColumn == null) {
            throw( new BistroException(BistroErrorCode.DEFINITION_ERROR, "Table operation error.", "A range table must have at least one attribute column for storing range values.")) ;
        }
        ...
        if(this.rangeColumn.getOperationType() != OperationType.ATTRIBUTE) {
            throw( new BistroException(BistroErrorCode.DEFINITION_ERROR, "Table operation error.", "A column for storing range values must be ATTRIBUTE column.")) ;
        }
    }
