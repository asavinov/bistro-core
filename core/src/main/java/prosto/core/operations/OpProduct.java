...
public class OpProduct implements Operation {
        ...
        while (top >= 0) {
            if (top == colCount) // New element is ready. Process it.
            {
                ...
                try {
                    ...
                    throw( new ProstoException(ProstoErrorCode.EVALUATION_ERROR, e.getMessage(), "Error checking where condition.") );
                }
    ...
    public boolean isWhereTrue(List<Object> record, List<Column> columns) {
        ...
        try {
            ...
            throw( new ProstoException(ProstoErrorCode.EVALUATION_ERROR, e.getMessage(), "") );
        }
